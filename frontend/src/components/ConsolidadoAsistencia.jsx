...
const ConsolidadoAsistencia = () => {
  ...
  const [seleccion, setSeleccion] = useState({ 
      grado: '', // Inicializamos vacío para esperar la carga del perfil
      seccion: '', 
  ...
  const areasConfig = {
   ...
   useEffect(() => {
    const configurarComponente = async () => {
      try {
        const { data: { user } } = await supabase.auth.getUser();
        if (!user) return;

        // Paralelismo para evitar lentitud en la red
        const [perfilRes, asignacionesRes] = await Promise.all([
          supabase.from('usuarios').select('*').eq('id_usuario', user.id).single(),
          supabase.from('docente_asignaciones').select('grado, seccion, area').eq('id_usuario', user.id)
        ]);

        if (perfilRes.data) {
          const perfil = perfilRes.data;
          const esAdmin = Number(perfil.rol_id) === 1;
          let gradosFinales = [];
          let areasFinales = [];

          if (esAdmin) {
            gradosFinales = opcionesGradoSeccion.map(o => `${o.g}-${o.s}`);
            areasFinales = Object.keys(areasConfig);
          } else {
            const asignaciones = asignacionesRes.data || [];
            gradosFinales = [...new Set(asignaciones.map(a => 
              `${a.grado.replace('°', '').trim()}-${a.seccion?.trim() || 'A'}`
            ))];
            areasFinales = [...new Set(asignaciones.map(a => a.area.trim()))];
          }

          const [g, s] = gradosFinales[0]?.split('-') || ['', ''];
          
          // Actualización de estado atómica (Batching de React 18)
          ...
          setIsReady(true); // Finaliza el estado de carga inicial
        }
      } catch (err) {
  ...
  const cargarDatos = useCallback(async () => {
    if (!isReady || !seleccion.grado || !perfilUsuario) return;
    
    setLoading(true);
    try {
      const gradoFmt = `${seleccion.grado}°`;
      
      const [nominaRes, asistenciasRes] = await Promise.all([
      ...
      setEstudiantes(nominaRes.data?.map(est => ({
        ...est,
        nombreCompleto: `${est.apellido_paterno} ${est.apellido_materno}, ${est.nombres}`.toUpperCase()
      })) || []);

      // Lógica de mapeo para el objeto 'datos'
      const mapaAsistencias = {};
      asistenciasRes.data?.forEach(asist => {
        if (!mapaAsistencias[asist.id_estudiante]) mapaAsistencias[asist.id_estudiante] = {};
        const dia = new Date(asist.fecha).getUTCDate();
        mapaAsistencias[asist.id_estudiante][dia] = asist.estado;
      });
      setDatos(mapaAsistencias);
  ...
  }, [isReady, seleccion.grado, seleccion.seccion, seleccion.area, perfilUsuario]);
      ...
      <div className="mb-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest">
            Registro Auxiliar 2026 | {perfilUsuario?.rol_id === 1 ? 'ADMINISTRADOR' : 'DOCENTE'}
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2 w-full lg:w-auto">
          ...
          <div className="min-w-[120px] flex items-center gap-2 bg-slate-100 px-3 py-2 rounded-2xl border">
            ...
            <select 
              ...
              onChange={(e) => {
                ...
                setSeleccion({...seleccion, grado: g, seccion: s});
              }}
            >
              {opcionesPermitidas.grados.map(opt => (
                <option key={opt} value={opt}>{opt.replace('-', '° ')}</option>
              ))}
              {opcionesPermitidas.grados.length === 0 && <option>CARGANDO...</option>}
            </select>
          </div>
          ...
          <div className="flex-1 min-w-[150px] flex items-center gap-2 bg-slate-100 px-3 py-2 rounded-2xl border">
            ...
            <select 
              ...
              onChange={(e) => setSeleccion({...seleccion, area: e.target.value})}
            >
              {opcionesPermitidas.areas.map(a => <option key={a} value={a}>{a}</option>)}
              {opcionesPermitidas.areas.length === 0 && <option value="">SIN ASIGNACIÓN</option>}
            </select>
          </div>
          ...
          <select 
            ...
            value={seleccion.mes} 
            onChange={(e) => setSeleccion({...seleccion, mes: parseInt(e.target.value)})}
          >
                  ...
                  return (
                    <tr key={est.id_matricula} className="hover:bg-slate-50 transition-colors group">
                      ...
                      {diasDelMes.map(dia => {
                        ...
                        const esFinde = dia.nombre === "Sáb" || dia.nombre === "Dom";
                        return (
                          <td key={dia.numero} className={`text-center py-2 text-[11px] border-r border-slate-300/80 ${esFinde ? 'bg-red-50/50 text-red-400' : 'bg-white text-slate-400'}`}>
                            {res}
                          </td>
                        );
                      })}
